...
static cl::opt<std::string> OutputFilename("bin",
...
static uint16_t encodeType(Type *T, bool recurse = true) {
  ...
  case Type::TypeID::PointerTyID:
    if (recurse) {
      TypeEncoded =
          uint16_t(16) +
          encodeType(dyn_cast<PointerType>(T)->getElementType(), false);
    } else {
      TypeEncoded = 11;
    }
    break;
...
bool isBlackListed(const Function &F) {
  return (F.getName().startswith("llvm.") || F.getName().startswith("__") ||
          F.getName() == "_Znwm");
}
...
std::string createDirectory(const std::string &currPath) {
  if (!sys::fs::is_directory(currPath + "/SDOutput") ||
      !sys::fs::exists(currPath + "/SDOutput")) {
...
std::string getOutputBinName() {
  if (OutputFilename.empty()) {
    ...
    if (!binPath) {
      errs()
          << "[BinaryCFI Error] (env set) Environment variable is not set!\n";
      return "";
    }
...
std::pair<std::string, std::string> getOutputFileName(Module &M,
  ...
  std::string CallsiteFileNameExtended =
      (CallsiteFileName + Twine(count) + ".csv").str();
...
void storeAdditionalData(Module &M, const std::string key) {
  ...
  raw_fd_ostream OutfileCallsite(FileNames.second, ECCallsite,
                                 sys::fs::OpenFlags::F_None);
  if (ECFun || ECCallsite) {
    errs() << "Failed to write to " << FileNames.first << ", "
           << FileNames.second << "!\n";
    return;
  }

  errs() << "Writing function and callsite encoding data to " << FileNames.first
         << ", " << FileNames.second << ".\n";
...
static std::stringstream writeDebugLocToStream(const DebugLoc *Loc) {
  ...
  Stream << Scope->getFilename().str() + ":" << Loc->getLine() << ":"
         << Loc->getCol();
  return Stream;
}
...
void visitor(Module &M) {
  ...
  storeAdditionalData(M, std::to_string(total_callsites) + "_" +
                             std::to_string(total_funs));
}
...
static RegisterPass<LegacyBinaryCFI>
    X("legacy-binary-cfi", "Binary CFI",
...
static llvm::RegisterStandardPasses
    RegisterBinaryCFI(PassManagerBuilder::EP_FullLinkTimeOptimizationLast,
                      loadPass);
